...
class GeneralController extends CourseSearchController
{
    ...
    public function index(Request $request) {
        parent::index();
        $this->general->lists = DB::table('courses')
            ->where('unit_name', (DB::table('students')
            ->where('id', Auth::user()->id)->get()[0]->unit_name))
            ->get();
